...
predictDataprep <- function(x, data)
{
  ...
  setDF(data)
  factor_flag <- x$var_list$first_set_var$A
  date_flag <- x$var_list$first_set_var$B
  ...
  freq_vklist <- x$var_list$first_set_var$I
...
if( length(var_not_found) > 0){ cat("List of variable's are not found in prediction data set: ", paste(var_not_found))
} else {
  master_data = XX[fvar]
  return(master_data)
}
